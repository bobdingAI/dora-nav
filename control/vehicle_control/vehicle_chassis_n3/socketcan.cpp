...
int write_socketcan_frame(const int32_t fd,const struct can_frame &frame){
  // std::cout << "frame:" << (float)frame.data[0] << "," << (float)frame.data[1] << std::endl;
    int byte_num = sizeof(struct can_frame);
    if (byte_num != write(fd,&frame,sizeof(struct can_frame))){
      int n = write(fd,&frame,sizeof(struct can_frame));
      // std::cout << "write size:" << (float)n << std::endl;
      perror("frame send err");
      return -1;
    }
    return 0;
}
