...
//-->>静态全局变量
static struct VehPara veh_para = {1.8, 1.2, 0.67, 500, 1.6, 1.6, 1.6, 34.2, 45, 1024, 0.6};
// static struct VehPara veh_para = {0, 0, 0, 0, 0, 0, 0, 0, 28, 0, 0};
...
static float ackermann_steering(const Eigen:: Vector2f &goal_point,const float La){
    ...
    // std::cout << "veh_para.max_wheel_angle: " << veh_para.max_wheel_angle << "\n deltai is(rad): " << deltai*180/M_PI << std::endl;

    //->内轮转角弧度转化为度
    // if(deltai*180/M_PI > 28)                          return 25.0f;
    // if(deltai*180/M_PI < -28)                         return -25.0f;
    return deltai*180/M_PI;
