...
int run(void *dora_context)
{
    ...
    while(!to_exit_process)
    {
        ...
        if (ty == DoraEventType_Input)
        {
            ...
            // RS_MSG << "sizeof(PointT) <= 16 " << RS_REND;
            size_t cloudSize = (((msg->points.size()) + 1) * 16);  // 4byte for message seq, 4bytes empty, 8byte for timestamp, others for points
                                                                // others for points
            u_int8_t* bytePointCloud = (u_int8_t*)(new PointT[cloudSize / sizeof(PointT)]);
            ...
            *seq = msg->seq;
            ...
            uint64_t* timestamp = (uint64_t*)(bytePointCloud + 8);
            uint64_t seconds = msg->timestamp * 1e6;
            *timestamp = seconds;
...
int main()
{
    ...
    param.input_param.msop_port = std::getenv("LIDAR_MSOP_PORT") ? std::stoi(std::getenv("LIDAR_MSOP_PORT")) : 6699;
    param.input_param.difop_port = std::getenv("LIDAR_DIFOP_PORT") ? std::stoi(std::getenv("LIDAR_DIFOP_PORT")) : 7788;
    std::cout << "param.input_param.msop_port: " << param.input_param.msop_port << " param.input_param.difop_port: " << param.input_param.difop_port << std::endl; 
    ...
    if (lidar_type_env == "RSHELIOS_16P") {
        param.lidar_type = LidarType::RSHELIOS_16P;
    } else if (lidar_type_env == "RSHELIOS") {
        param.lidar_type = LidarType::RSHELIOS;
    } else {
        std::cerr << "Please add lidar, now using default RSHELIOS_16P" << std::endl;;
        param.lidar_type = LidarType::RSHELIOS_16P;
    }
    ...
    // param.lidar_type = LidarType::RSHELIOS_16P;   ///< Set the lidar type. Make sure this type is correct雷达类型
